...
class Args {
     ...
     * @returns The tokens.
     */
    manyFromEnd(limit = Infinity, from = this.state.positionFromEnd) {
        const ts = [];
     ...
     */
    findMap(f, from = this.state.position) {
     ...
     * @returns The resulting value if it was found.
     */
    async findMapAsync(f, from = this.state.position) {
        for (let i = from; i < this.length; i++) {
            if (this.state.usedIndices.has(i)) {
                continue;
            }
     ...
     */
    findParse(f, from = this.state.position) {
        const errors = [];
        for (let i = from; i < this.length; i++) {
            if (this.state.usedIndices.has(i)) {
                continue;
     ...
     * @returns The resulting value if it was found or a list of errors during parsing.
     */
    async findParseAsync(f, from = this.state.position) {
        const errors = [];
        for (let i = from; i < this.length; i++) {
            if (this.state.usedIndices.has(i)) {
                continue;
     ...
     */
    filterMap(f, limit = Infinity, from = this.state.position) {
        const ys = [];
     ...
     * @returns The resulting values.
     */
    async filterMapAsync(f, limit = Infinity, from = this.state.position) {
        const ys = [];
        for (let i = from; i < this.length && ys.length < limit; i++) {
            if (this.state.usedIndices.has(i)) {
                continue;
